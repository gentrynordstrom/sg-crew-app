      ...
      boards(ids: [$boardId]) {
        items_page(limit: 100, query_params: {
          ...
          items {
            ...
            name
            column_values(ids: [
              "${CLEANING.columns.date.id}",
              "${CLEANING.columns.cleaningType.id}",
              "${CLEANING.columns.maintenanceNeeded.id}",
              "${CLEANING.columns.crewMember.id}"
            ]) {
              id
              text
            }
    ...
    <main className="min-h-screen bg-brand-moss-700 px-4 py-8">
      <div className="mx-auto max-w-2xl">
        <LogPageHeader
          ...
          newHref="/cleaning-log/new"
        />

        {items.length === 0 ? (
          <div className="rounded-2xl bg-brand-moss-600/40 p-8 text-center text-brand-cream-400">
            No entries yet.{" "}
            <Link href="/cleaning-log/new" className="text-brand-brass-300 underline underline-offset-4">
              Add the first one.
            </Link>
          </div>
        ) : (
          <ul className="space-y-3">
            {items.map((item) => {
              const date = col(item, CLEANING.columns.date.id);
              const cleaningType = col(item, CLEANING.columns.cleaningType.id);
              const maintenanceNeeded = col(item, CLEANING.columns.maintenanceNeeded.id);
              const crew = col(item, CLEANING.columns.crewMember.id);
              return (
                <li key={item.id}>
                  <Link
                    href={`/cleaning-log/${item.id}`}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        {maintenanceNeeded === "Yes" && (
                          <StatusChip label="Maintenance Needed" variant="warning" />
                        )}
                      </div>
                      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-brand-cream-400">
                        {date && <span>{date}</span>}
                        {cleaningType && <span>{cleaningType}</span>}
                        {crew && <span>{crew}</span>}
                      </div>
                    </div>
                    <ChevronRightIcon />
