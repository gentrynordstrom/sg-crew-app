...
3. `/login` shows "Sign in with Google" button → Supabase OAuth flow
4. Google callback hits `/auth/callback/route.ts` → Supabase exchanges code for session → cookies set
...
2. Project Settings → Auth → Providers → enable Google
   - Need a Google Cloud OAuth client first — see step B
3. Copy the URL, anon key, and service role key into `.env.local`
4. Copy the Postgres connection strings (pooler + direct) into `.env.local`
...
2. Sign in with `gentry@stegenriverboat.com` → trigger auto-promotes you to ADMIN
3. Land on `/` → see all feature tiles + admin tile
4. Navigate to `/admin/users` → see yourself in the list
5. Sign out from incognito → sign in with a test Google account → lands on `/pending`
...
4. /login, /pending, /, /coming-soon, /admin/users pages
5. /api/admin/set-role endpoint (admin-only)
6. PWA manifest + icons
7. Feature tiles on / that link to /coming-soon for now
